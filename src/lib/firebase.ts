...
const hasConfig = missingFirebaseEnv.length === 0;

if (!hasConfig) {
  console.warn(
    `[firebase] Configuracao incompleta. Variaveis faltando: ${missingFirebaseEnv.join(", ")}`
  );
}

const app = hasConfig && getApps().length === 0 
